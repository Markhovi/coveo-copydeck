/**
 * docx-builder.js — pure-JS ZIP/DOCX generator, zero dependencies.
 *
 * Depends on: schema.js (SCHEMA.docxSections, SCHEMA.fields, SCHEMA.widgetRows)
...
const DocxBuilder = (() => {
  ...
  function buildZip(files) {
    ...
    for (const [name, content] of files) {
      const nb = bytes(name);
      const cb = typeof content === 'string' ? bytes(content) : content;
      const crc = crc32(cb);
