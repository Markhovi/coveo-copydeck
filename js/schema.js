/**
 * schema.js — single source of truth for all event fields.
 *
 * Every field defined here automatically appears in:
 *   - The rendered form (form.js)
 *   - The .docx copydeck (docx-builder.js)
 *   - The Sitecore CSV export (csv-builder.js)
 *
 * To add a field: add it to SCHEMA.fields and reference it in
 * the appropriate section's rows array.
 */
...
const SCHEMA = {
  ...
  /* ── Field definitions ──────────────────────────────────────────────
   *
   * Each field key matches the name used in collectForm() state.
   *
   * Field properties:
   ...
   * componentWidth is intentionally absent (removed from spec).
   */
  widgetRows: [
    {
      cols: 2,
